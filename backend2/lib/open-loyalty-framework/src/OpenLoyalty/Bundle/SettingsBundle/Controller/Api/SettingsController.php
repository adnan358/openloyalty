...
class SettingsController extends FOSRestController
{
    ...
    /**
     * Method will update hotelrunner entry.
     *
     * @Route(name="oloy.settings.update_hotelrunner", path="/settings/hotelrunner")
     * @Method("PUT")
     * @Security("is_granted('EDIT_SETTINGS')")
     * @ApiDoc(
     *     section="Settings"
     ...
     * @return View
     */
    public function updateHotelRunnerEntryAction(Request  $request): View
    {
        $settingsManager = $this->get('ol.settings.manager');
        $settings = $settingsManager->getSettings();
        $entry = $settings->getEntry('hotelrunner');
        if (!empty($entry)) {
            $settings->getEntry('hotelrunner')->setValue($request->get('value'));
            $this->get('ol.settings.manager')->save($settings);
